...
export function ReelCard({
  property, index, isActive, isFollowing, isFavorite,
  onOpenDetails, onOpenSearch, onOpenSeller, onToggleFollow, onToggleFavorite, onShare,
}: Props) {
  ...
  const [likedByMe, setLikedByMe] = useState(!!property.likedByMe);
  ...
  return (
    <View style={{ height: reelHeight, backgroundColor: "#000" }}>
      {mode === "video" && video ? (
        ...
        <Image source={{ uri: images[slideIdx].url }} style={StyleSheet.absoluteFill} contentFit="cover" />
      ) : (
      ...
      <ReelInfoOverlay property={property} onOpenDetails={onOpenDetails} />
