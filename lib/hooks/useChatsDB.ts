...
async function fetchChatList(userId: string): Promise<ChatSummary[]> {
  ...
  return rows.map((r) => {
    ...
    const partnerName = partnerProfile?.full_name || "مستخدم ديار توك";
...
export function useMarkChatRead() {
  ...
  return useMutation({
    mutationFn: async ({ chatId, currentUserId }: { chatId: string; currentUserId: string }) => {
      const { error } = await supabase.from("chat_messages").update({ read: true }).eq("chat_id", chatId).neq("sender_id", currentUserId);
      if (error) throw error;
    },
