...
export function ReelInfoOverlay({ property, onOpenDetails }: { property: Property; onOpenDetails: () => void }) {
  const p = property;
  const { t } = useLanguage();
  return (
    ...
    >
      <Pressable style={styles.infoBlock} onPress={onOpenDetails}>
        ...
        <Text style={styles.title} numberOfLines={1}>{p.shortTitle || p.title}</Text>
