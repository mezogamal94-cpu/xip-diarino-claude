...
export function MakeOfferModal({ visible, request, onClose, onSubmit }: Props) {
  ...
  function reset() {
    setMessage(""); setPrice(""); setWhatsapp(""); setWhatsappError(false); setMessageError(false);
  }

  function submit() {
    // ↔ submitOffer() validation: message required, whatsapp required & >= 8 chars
    if (!message.trim()) { setMessageError(true); return; }
    if (!whatsapp.trim() || whatsapp.trim().length < 8) { setWhatsappError(true); return; }
    onSubmit(message.trim(), price.trim(), whatsapp.trim());
    reset();
  }
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={styles.sheet}>
        ...
        <TextInput
          style={[styles.input, whatsappError && styles.inputError]}
          value={whatsapp}
          onChangeText={(v) => { setWhatsapp(v); setWhatsappError(false); }}
