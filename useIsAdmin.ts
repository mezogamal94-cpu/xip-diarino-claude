...
export function SearchFilterModal({ visible, value, onApply, onClose }: Props) {
  ...
  const availableRegions = draft.provinces.length === 1 ? regionsForProvince(draft.provinces[0]) : [];
            ...
            <>
              ...
              <View style={styles.chipsRow}>
                {availableRegions.map((r) => (
                  <Pressable
                    key={r}
                    style={draft.regions.includes(r) ? styles.chipActive : styles.chip}
                    onPress={() =>
                      setDraft((d) => ({
                        ...d,
                  ...
                  >
                    <Text style={draft.regions.includes(r) ? styles.chipActiveText : styles.chipText}>{t(r)}</Text>
                  </Pressable>
                ))}
              </View>
            </>
